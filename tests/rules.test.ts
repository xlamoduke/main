...
it('A♠ бьёт K♠', () => assert.equal(beatsCard(cards('KS')[0], cards('AS')[0], heartsTrump), true));
it('K♠ не бьёт A♠', () => assert.equal(beatsCard(cards('AS')[0], cards('KS')[0], heartsTrump), false));
it('6♥ как козырь бьёт A♠, если hearts — козырь', () => assert.equal(beatsCard(cards('AS')[0], cards('6H')[0], heartsTrump), true));
it('A♠ не бьёт 6♥, если hearts — козырь', () => assert.equal(beatsCard(cards('6H')[0], cards('AS')[0], heartsTrump), false));
it('в no-trump A♠ бьёт K♠', () => assert.equal(beatsCard(cards('KS')[0], cards('AS')[0], noTrump), true));
it('в no-trump 7♠ бьёт 6♠', () => assert.equal(beatsCard(cards('6S')[0], cards('7S')[0], noTrump), true));
it('в обычном режиме Shaman 6♠ нельзя побить ничем', () => {
  const shamanAttack = cards('6S')[0];
  for (const response of cards('AS 10S KS AH 6H 7S')) assert.equal(beatsCard(shamanAttack, response, heartsTrump), false);
});
